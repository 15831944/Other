...
namespace HVKApi
{
    public class CHVKDbObject
    {
	    ...
	    bool GetData()
        {	bool stat = true;
            stat = m_ptrAdoRc.GetData("ID", CADORecord.Types.Integer, m_ID);
	        if(stat)
                stat = m_ptrAdoRc.GetData("Name", CADORecord.Types.String, m_Name);
	        return stat;
        }
