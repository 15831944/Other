...
int CIfcWnd::ShowHBDataStr(char* Capt, void* DataW, void* DataS, int DataType, int ShowType /* = BousHandlEdt*/,
								BtnFunct* ClickFnW /* = NULL*/, BtnFunct* ClickFnS/* = NULL*/, CADORecord* ptrCurrentRecordset)
{
	int y = m_yCtrl;
	int h = HCONTROL;
...
int CIfcWnd::ShowDataStr(char* Capt, void* Data, int DataType, int IsReadOnly/* = 0*/)
{
...
int CIfcWnd::ShowDataStr(char* Capt, void* Data, int DataType, 
						 ...
						 int dxBtn, int dyBtn, int IsReadOnly/* = 0*/)
{
	m_pObjArr[m_NumObj] = new CIfcEdit(&CIfcValue(Data, DataType, Capt), 
								m_xVal, m_yVal+m_NumObj*m_Offset, 0, 0, m_xStc, m_yStc+m_NumObj*m_Offset, 0, 0, IsReadOnly);
...
// текст + комбобокс
int CIfcWnd::ShowDataStr(char* Capt, void* Data, int DataType, CADORecord* ptrCurrentRecordset)
{
