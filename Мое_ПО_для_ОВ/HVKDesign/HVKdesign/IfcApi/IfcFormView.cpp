...
static int CalcHL(void* Data)
{
	...
	pRm->Update();
	pFv->Update();
	return AfxMessageBox("Расчет теплопотерь помещения");
};
...
static int ShowAirIn(void* Prm)
{
	...
	Dlg.ShowBorderStc = TRUE;
	Dlg.ShowHBDataStr("Приточная вентиляция, м3/ч", 
		(void*)&pRm->m_LInSum, (void*)&pRm->m_LInSum, 
		CIfcValue::Double, CIfcWnd::DontEdt);
	Dlg.ShowHBDataStr("                механическая на душирование", 
...
static int ShowAirOut(void* Prm)
{
	...
	Dlg.ShowBorderStc = TRUE;
	Dlg.ShowHBDataStr("Вытяжная вентиляция, м3/ч", 
...
//Возмещение тепла системой отопления
	ShowHBDataStr("Тепло системы отопления, ккал/ч", 
...
static int CalcFenceHL(void* Prm)
{
	...
	if(pFnc->CalcHeatLoss())
	{
		...
		AfxMessageBox("Расчет теплопотерь ограждения завершен");
	}
	return TRUE;
}
