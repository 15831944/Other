...
COMAPI void GenerateError(HRESULT hr, PWSTR pwszDescription, char* ExtStr)
{
char strError[256];
  switch (hr)
  {
	case -2146824556:
      strcpy(strError, "Сервер не может завершить операцию");
      break;
    case -2146825023:
      strcpy(strError, "Ошибка в структуре базы данных\nНужного поля не существует");
      break;
	case -2146825267:
      strcpy(strError, "Запись не найдена\nДополнительные данные: %s", ExtStr);
      break;
	case -2147217843:
      strcpy(strError, "Неверное имя пользователя и пароль\nПриложение будет закрыто");
      break;
    case -2147217873:
      strcpy(strError, "Операция не выполнена\nНельзя удалить запись на которую есть ссылка в другой таблице");
      break;
    case -2147217911:
      strcpy(strError, "У Вас нет прав на выполнение этой операции");
      break;
    default:
