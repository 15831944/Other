...
CIfcEdit::CIfcEdit(CIfcValue* Value, int xVal , int yVal, int dxVal/* = 0*/, int dyVal/* = 0*/,
		 ...
		 int IsReadOnly/* = 0*//*, EdtFunct* UpdateFn = NULL*/)
		 :CIfcObject(Value, xVal , yVal, dxVal, dyVal, xCapt, yCapt, 
		  dxCapt, dyCapt), m_IsReadOnly(IsReadOnly)/*, m_UpdateFn(UpdateFn)*/
{
...
void CIfcEdit::OnKillfocus() 
{
//	if(m_UpdateFn)
//		(m_UpdateFn->m_Fn)(m_UpdateFn->m_Data);
}
