...
CProject* CProject::Copy(CProject const& op2)
{
	...
	m_pBuildingVect.ChangeParent(this);
	if(op2.m_pCity)
		*m_pCity        = *op2.m_pCity;
...
//---------------------------------------------------------------------------
int CProject::GetSvPrj()
{
	*this = *m_svPrj;
	return TRUE;
}
//---------------------------------------------------------------------------
int CProject::IsNewPrj()
{
	return !strcmp(m_svPrj->m_Name, "");
}
//---------------------------------------------------------------------------
int CProject::IsChangePrj()
{
	return !(m_svPrj && *m_svPrj == *this && !IsNewPrj());
}
//---------------------------------------------------------------------------
...
/*
		CADORecord ptrAdoRc(CHVKDbObject::m_ptrAdoRc->m_ptrConnection);
		SaveCurADORecord();
...
/*
	...
	CADORecord* AdoRcPrj = CHVKDbObject::m_ptrAdoRc;
	char str[256];
	...
	(static_cast<CIfcComboBox*>(Dlg.m_pObjArr[Dlg.m_NumObj]))->SetCanDelete(TRUE);
	Dlg.m_NumObj++;
...
{
	if(!IsNew && !strcmp(m_Name, ""))
	{	MessageBox(NULL, "Укажите название проекта",
			       "Сохранение проекта", MB_OK|MB_ICONERROR);
		return FALSE;
	}
	try{
		_bstr_t bstrEmpty(L"");
