...
void CZdanie::SetData()
{
	if(!Name) return;
	m_Dlg.Clear();
	if(KoefNaRaznNaklRash!=-1) m_Dlg.m_K1.Format("%f", KoefNaRaznNaklRash);
	if(KoefPereschetOborud!=-1)m_Dlg.m_K2.Format("%f", KoefPereschetOborud);
	if(KoefPereschetCMR!=-1) m_Dlg.m_K3.Format("%f", KoefPereschetCMR);
	if(V!=-1)m_Dlg.m_V.Format("%f", V);
	m_Dlg.m_N = Name;
}
...
void CZdanie::Calculate(CRazdelZdanie* Razdel)
{
	...
	Razdel->SetVsegoStstOborudov(Razdel->GetAnalog().GetVsegoStstOborudov()*KoefPereschetOborud*
							     V/Razdel->GetAnalog().GetV());
	Razdel->SetVsegoStstMontaga(Razdel->GetAnalog().GetVsegoStstMontaga()*KoefPereschetCMR*
								V/Razdel->GetAnalog().GetV());
	Razdel->SetVsegoStstSantehRabot(Razdel->GetAnalog().GetVsegoStstSantehRabot()*KoefPereschetCMR*
								 V/Razdel->GetAnalog().GetV());
	Razdel->SetVsegoStstObshestrRabot(Razdel->GetAnalog().GetVsegoStstObshestrRabot()*KoefPereschetCMR*
								   V/Razdel->GetAnalog().GetV());
...
void CZdanie::Read(::fstream& f) 
{
	...
	while(!f.eof())
	{
		r=new CRazdelZdanie();
		f>>(*r);
		if(!f.eof()) TableOfRazdel.Add(r);
		else delete r;
	}
}

void CZdanie::Write(::fstream& f)
{
...
//перейти к следующей ячейке и ввести текст
	oSel.MoveRight(COleVariant(short(12)), covOpt, covOpt);
	str.Format("%5.2f",pRazdel->GetSumStoimost());//суммарная стоимость
	oSel.TypeText(str);
	//установить выравнивание по центру
	oParFormat= oSel.GetParagraphFormat();
	oParFormat.SetAlignment(1);
//перейти к следующей ячейке и ввести текст
	...
	str.Format("%5.2f",
		pRazdel->GetVsegoStstObshestrRabot()+pRazdel->GetVsegoStstSantehRabot());
	oSel.TypeText(str);
	//установить выравнивание по центру
	oParFormat= oSel.GetParagraphFormat();
	oParFormat.SetAlignment(1);
//перейти к следующей ячейке и ввести текст
	...
	str.Format("%5.2f",pRazdel->GetVsegoStstMontaga());
	oSel.TypeText(str);
	//установить выравнивание по центру
	oParFormat= oSel.GetParagraphFormat();
	oParFormat.SetAlignment(1);
//перейти к следующей ячейке и ввести текст
	...
	str.Format("%5.2f",pRazdel->GetVsegoStstOborudov());
	oSel.TypeText(str);
	//установить выравнивание по центру
	oParFormat= oSel.GetParagraphFormat();
	oParFormat.SetAlignment(1);
...
//		app.SetVisible(TRUE); //и сделать его видимым
	...
	//	oFont.SetName("Arial");	//стиль шрифта Arial
		...
		str.Format("%5.2f",SumStoimost);	//суммарная стоимость
		oSel.TypeText(str);
		//установить выравнивание по центру
		oParFormat= oSel.GetParagraphFormat();
		...
		oSel.MoveRight(COleVariant(short(12)), covOpt, covOpt);
		//стоимость строит.работ
		str.Format("%5.2f", SumVsegoStstObshestrRabot+SumVsegoStstSantehRabot);
		oSel.TypeText(str);
		//установить выравнивание по центру
		oParFormat = oSel.GetParagraphFormat();
		oParFormat.SetAlignment(AL_CENTER);
		...
		oSel.MoveRight(COleVariant(short(12)), covOpt, covOpt);
		//стоимость монтажных работ
		str.Format("%5.2f", SumVsegoStstMontaga);
		oSel.TypeText(str);
		//установить выравнивание по центру
		oParFormat= oSel.GetParagraphFormat();
		oParFormat.SetAlignment(AL_CENTER);
		...
		oSel.MoveRight(COleVariant(short(12)), covOpt, covOpt);
		//стоимость оборудования
		str.Format("%5.2f", SumVsegoStstOborudov);
		oSel.TypeText(str);
		//установить выравнивание по центру
		oParFormat= oSel.GetParagraphFormat();
		oParFormat.SetAlignment(AL_CENTER);
